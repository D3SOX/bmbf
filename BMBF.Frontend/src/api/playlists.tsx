...
export async function fetchPlaylists(): Promise<void> {
  const data = await backendRequest(`${API_ROOT}/playlists`);
  if (data.ok) {
...
export async function getPlaylistSongs(
  ...
  const data = await backendRequest(`${API_ROOT}/playlists/songs/${playlist.id}`);
  if (data.ok) {
    return data.json();
  }
}
...
export async function deletePlaylist(playlist: Pick<Playlist, 'id'>): Promise<void> {
  const data = await backendRequest(`${API_ROOT}/playlists/delete/${playlist.id}`, {
    method: 'DELETE',
  });
}
